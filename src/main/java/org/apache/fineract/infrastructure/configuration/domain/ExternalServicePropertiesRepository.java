...
/**
 * Tenant-aware data access for Fineract external service configuration tables.
 * Uses the current tenant connection (RoutingDataSource / ThreadLocal context).
 */
public interface ExternalServicePropertiesRepository {

  /**
   * Returns all property rows for the given external service name in the current tenant.
   * Empty list if the service is not registered or has no properties.
   */
