...
public class SelfAccountTransferWritePlatformServiceImpl implements SelfAccountTransferWritePlatformService {
    ...
    public CommandProcessingResult executeInternalTransfer(AccountTransferConfirmRequest request) {
        ...
        LocalDate transferDate = LocalDate.parse(request.getTransferDate(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        ...
        log.info("Executing internal transfer from account {} to account {} for amount {}", 
                request.getFromAccountId(), request.getToAccountId(), request.getTransferAmount());
        ...
        log.info("Internal transfer completed successfully with transaction ID: {}", transferTransactionId);
