...
public class PucSavingsAccountActivationListener {
  ...
  @PostConstruct
  public void init() {
    // Guard against empty property values overriding the defaults
    if (!StringUtils.hasText(eventTopicName)) {
      eventTopicName = "fineract.external.events";
    }
    if (!StringUtils.hasText(subscriptionName)) {
      subscriptionName = "selfservice-puc-activation-notifications";
    }
  ...
  private void processSavingsActivationEvent(SavingsAccountDataV1 accountDataV1) {
    ...
    if (accountDataV1.getId() != null) {
      try {
        ...
        log.error("Could not parse savings account ID from Avro: {}", accountDataV1.getId());
        return;
      }
    }
    ...
    log.info(
        "Captured [SavingsActivateBusinessEvent]. Starting automatic PUC account registration for ID: {}",
        savingsAccountId);

    try {
      // 1. Load savings account from Fineract
      SavingsAccountData accountData =
          this.savingsAccountReadPlatformService.retrieveOne(savingsAccountId);
      if (accountData == null) {
        log.warn("Savings account not found for ID: {}", savingsAccountId);
        return;
      }

      // 2. Load client to obtain identification / cédula
      ClientData clientData =
          this.clientReadPlatformService.retrieveOne(accountData.getClientId());
      if (clientData == null) {
        log.warn("Client data not found for ID: {}", accountData.getClientId());
        return;
      }

      // 3. Build PUC / KINDO request
      PucAddAccountRequest pucRequest = new PucAddAccountRequest();

      // IBAN (externalId of the account)
      ...
      // Holder ID Type (FISICA or JURIDICA)
      ...
      log.info(
          "Sending AddAccount to PUC -> IBAN: {}, AccountNo: {}, Holder: {}, HolderId: {}, HolderIdType: {}, Currency: {}, Type: CAR",
          iban,
          accountNumber,
          holder,
          holderId,
      ...
      // 4. Call PUC REST client
      String response = this.pucExternalApiClient.addAccount(pucRequest);
      log.info("PUC response for IBAN {}: {}", iban, response);

    } catch (Exception e) {
      log.error(
          "Error registering account in PUC after SavingsActivateBusinessEvent", e);
    }
  }

  /**
   * Determines the identification type (FISICA or JURIDICA) from the client's legal form.
   */
  ...
  /**
   * Safely extracts the string value from an ExternalId or String object.
   */
