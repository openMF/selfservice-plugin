...
public class SelfServiceAccountForFeesRepository {
    ...
    /**
     * Fetches properties for a given external service name.
     * Multi-tenancy is handled automatically by Fineract's TenantDataSource routing.
     */
    public Map<String, String> getProperties(String serviceName) {
        ...
        try {
            jdbcTemplate.query(sql, new Object[]{serviceName}, rs -> {
                properties.put(rs.getString("name"), rs.getString("value"));
            });
        } catch (Exception e) {
            log.error("Error fetching external service properties for {}: {}", serviceName, e.getMessage());
        }
        return properties;
    }
}