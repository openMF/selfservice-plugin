...
@Slf4j
public class SelfServiceSinpeEnrollmentWritePlatformServiceImpl implements SelfServiceSinpeEnrollmentWritePlatformService {
    ...
    public CommandProcessingResult requestEnrollment(String mobileNumber) {
        ...
        if (sinpeRepository.findByAppSelfServiceUserIdAndMobileNumber(user.getId(), mobileNumber)
                .filter(SelfServiceSinpeEnrollment::isVerified).isPresent()) {
            return new CommandProcessingResultBuilder().withEntityId(user.getId()).build(); // Already enrolled
        }
        ...
        SelfServiceRegistration request = SelfServiceRegistration.instance(
                client, client.getAccountNumber(), client.getFirstname(), client.getMiddlename(), client.getLastname(),
                mobileNumber, user.getEmail(), otp, otp, user.getUsername(), "SINPE_OTP",
                SelfServiceRequestType.SINPE_ENROLLMENT, expiry
        );
    ...
    public CommandProcessingResult confirmEnrollment(String mobileNumber, String otp) {
        ...
        SelfServiceRegistration request = registrationRepository.findTopByClient_IdAndRequestTypeAndAuthenticationTokenOrderByCreatedAtDesc(
        ...
        if (request == null || request.isConsumed() || request.isExpired(DateUtils.getLocalDateTimeOfSystem())) {
        ...
        SelfServiceSinpeEnrollment enrollment = sinpeRepository
                .findByAppSelfServiceUserIdAndMobileNumber(user.getId(), mobileNumber)
                .orElse(new SelfServiceSinpeEnrollment(user.getId(), request.getClient().getId(), mobileNumber));
