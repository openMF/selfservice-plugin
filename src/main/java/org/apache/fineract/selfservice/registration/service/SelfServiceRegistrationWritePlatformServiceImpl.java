...
public class SelfServiceRegistrationWritePlatformServiceImpl
  ...
  public SelfServiceRegistration createRegistrationRequest(String apiRequestBodyAsJson) {
    ...
    baseDataValidator.reset().parameter(SelfServiceApiConstants.accountNumberParamName).value(accountNumber).notNull().notBlank().notExceedingLengthOf(100);
    ...
    baseDataValidator.reset().parameter(SelfServiceApiConstants.middleNameParamName).value(middleName).ignoreIfNull().notExceedingLengthOf(100);
    ...
    baseDataValidator.reset().parameter(SelfServiceApiConstants.passwordParamName).value(password).matchesRegularExpression(validationPolicy.getRegex(), validationPolicy.getDescription()).notExceedingLengthOf(100);
    ...
    baseDataValidator.reset().parameter(SelfServiceApiConstants.authenticationModeParamName).value(authenticationMode).notBlank().isOneOfTheseStringValues(SelfServiceApiConstants.emailModeParamName, SelfServiceApiConstants.mobileModeParamName);

    String email = this.fromApiJsonHelper.extractStringNamed(SelfServiceApiConstants.emailParamName, element);
    baseDataValidator.reset().parameter(SelfServiceApiConstants.emailParamName).value(email).notNull().notBlank().notExceedingLengthOf(100);
    ...
    SelfServiceRegistration selfServiceRegistration = SelfServiceRegistration.instance(
        client, accountNumber, firstName, middleName, lastName, mobileNumber, email,
        authenticationToken, authenticationToken, username, encodePassword(password),
  ...
  public void validateForDuplicateUsername(String username) {
    ...
    if (isDuplicateUserName) {
      final StringBuilder defaultMessageBuilder = new StringBuilder("User with username ").append(username).append(" already exists.");
      throw new PlatformDataIntegrityException("error.msg.user.duplicate.username", defaultMessageBuilder.toString(), SelfServiceApiConstants.usernameParamName, username);
    }
  }

  // REMOVED: sendAuthorizationToken, sendAuthorizationMessage, sendAuthorizationMail

  private void throwExceptionIfValidationError(final List<ApiParameterError> dataValidationErrors, String accountNumber, String firstName, String middleName, String lastName, String mobileNumber, boolean isEmailAuthenticationMode) {
  ...
  public AppSelfServiceUser createSelfServiceUser(String apiRequestBodyAsJson) {
    ...
    try {
      ...
      AppSelfServiceUser appUser = new AppSelfServiceUser(client.getOffice(), user, allRoles, selfServiceRegistration.getEmail(), client.getFirstname(), client.getLastname(), null, passwordNeverExpire, isSelfServiceUser, clients, null);
  ...
  private void handleDataIntegrityIssues(final JsonCommand command, final Throwable realCause, final Exception dve, String username) {
    if (realCause.getMessage().contains("'username_org'")) {
      final StringBuilder defaultMessageBuilder = new StringBuilder("User with username ").append(username).append(" already exists.");
      throw new PlatformDataIntegrityException("error.msg.user.duplicate.username", defaultMessageBuilder.toString(), "username", username);
    }
  ...
  public SelfServiceRegistration selfEnroll(String apiRequestBodyAsJson) {
    ...
    final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
    final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("user");
    this.fromApiJsonHelper.checkForUnsupportedParameters(typeOfMap, apiRequestBodyAsJson, SelfServiceApiConstants.SELF_ENROLLMENT_DATA_PARAMETERS);
    JsonElement element = gson.fromJson(apiRequestBodyAsJson.toString(), JsonElement.class);
    ...
    String authenticationMode = this.fromApiJsonHelper.extractStringNamed(SelfServiceApiConstants.authenticationModeParamName, element);
    baseDataValidator.reset().parameter(SelfServiceApiConstants.authenticationModeParamName).value(authenticationMode).notBlank().isOneOfTheseStringValues(SelfServiceApiConstants.emailModeParamName, SelfServiceApiConstants.mobileModeParamName, SelfServiceApiConstants.anyModeParamName);
    ...
    if (!isEmailAuthenticationMode) {
      baseDataValidator.reset().parameter(SelfServiceApiConstants.mobileNumberParamName).value(mobileNumber).notBlank().validatePhoneNumber();
    } else if (mobileNumber != null) {
      baseDataValidator.reset().parameter(SelfServiceApiConstants.mobileNumberParamName).value(mobileNumber).validatePhoneNumber();
    }
    ...
    try {
      ...
      AppSelfServiceUser appUser = new AppSelfServiceUser(client.getOffice(), springConfigUser, Collections.singleton(ssRole), email, client.getFirstname(), client.getLastname(), null, true, true, Collections.emptyList(), null);
      ...
      SelfServiceRegistration registration = SelfServiceRegistration.instance(
          ...
          SelfServiceRequestType.ENROLLMENT, selfServiceAuthorizationTokenService.calculateExpiry(createdAt));
  ...
  public AppSelfServiceUser confirmEnrollment(String apiRequestBodyAsJson) {
    ...
    if (appUser == null) {
      throw new PlatformDataIntegrityException("error.msg.user.notfound.username", "User with username " + request.getUsername() + " not found.", SelfServiceApiConstants.usernameParamName, request.getUsername());
    }
    ...
    try {
      ...
      throw new PlatformDataIntegrityException("error.msg.self.service.request.token.invalid", "The supplied self-service token is expired or already used.", SelfServiceApiConstants.externalAuthenticationTokenParamName);
    }
    ...
    try { capturedTenant = ThreadLocalContextUtil.getTenant(); } catch (IllegalStateException ignored) {}
      ...
      public void afterCommit() {
        try {
          applicationEventPublisher.publishEvent(new SelfServiceNotificationEvent(
              SelfServiceRegistrationWritePlatformServiceImpl.this, SelfServiceNotificationEvent.Type.USER_ACTIVATED,
              appUser.getId(), appUser.getFirstname(), appUser.getLastname(), appUser.getUsername(),
              request.getEmail(), request.getMobileNumber(), isEmailMode(request), null, LocaleContextHolder.getLocale(),
              tenantSnapshot, businessDatesSnapshot));
        } catch (Exception e) {
  ...
  private void publishEnrollmentTokenEvent(SelfServiceRegistration registration, boolean isEmailMode) {
    ...
    try {
      java.util.HashMap<org.apache.fineract.infrastructure.businessdate.domain.BusinessDateType, java.time.LocalDate> dates = org.apache.fineract.infrastructure.core.service.ThreadLocalContextUtil.getBusinessDates();
    ...
    Runnable publishTask = () -> {
      try {
        applicationEventPublisher.publishEvent(new org.apache.fineract.selfservice.notification.SelfServiceNotificationEvent(
            ...
            org.apache.fineract.selfservice.notification.SelfServiceNotificationEvent.Type.ENROLLMENT_TOKEN,
            registration.getId(),
        ...
        log.warn("Failed to publish ENROLLMENT_TOKEN notification for registrationId={}", registration.getId(), e);
      }
    };
    ...
    if (org.springframework.transaction.support.TransactionSynchronizationManager.isSynchronizationActive()) {
      ...
      // Fallback: If no transaction is active (e.g., in createRegistrationRequest which uses saveAndFlush, 
      // or in unit tests), publish the event immediately since the data is already committed.
      publishTask.run();
    }
  }

  // ... (All other helper methods like resolveEnrollmentRequest, normalizeSelfEnrollmentClientPayload, etc. remain exactly the same) ...
  
  private SelfServiceRegistration resolveEnrollmentRequest(JsonElement element, DataValidatorBuilder baseDataValidator, List<ApiParameterError> dataValidationErrors) {
    ...
    if (externalToken != null) {
      baseDataValidator.reset().parameter(SelfServiceApiConstants.externalAuthenticationTokenParamName).value(externalToken).notBlank().notExceedingLengthOf(100);
      if (!dataValidationErrors.isEmpty()) return null;
      SelfServiceRegistration request = this.selfServiceRegistrationRepository.getRequestByExternalAuthorizationToken(externalToken, SelfServiceRequestType.ENROLLMENT);
    ...
    if (!dataValidationErrors.isEmpty()) return null;
    SelfServiceRegistration request = this.selfServiceRegistrationRepository.getRequestByIdAndAuthenticationToken(id, authenticationToken, SelfServiceRequestType.ENROLLMENT);
  ...
  private AppUser resolveEnrollmentAuditUser(String auditUsername) {
    try {
      ...
      if (auditUserId == null) throw new IllegalStateException("Self-enrollment audit user '" + auditUsername + "' is configured but no matching m_appuser id was returned.");
      return appUserRepository.findById(auditUserId).orElseThrow(() -> new IllegalStateException("Self-enrollment audit user '" + auditUsername + "' was found in m_appuser lookup but could not be loaded from the repository."));
    } catch (EmptyResultDataAccessException e) {
      throw new IllegalStateException("Configured self-enrollment audit user '" + auditUsername + "' does not exist. Set fineract.selfservice.enrollment.audit-user to a valid username.", e);
    }
  }

  private void validateForDuplicateUsernameForEnrollment(String username) {
    boolean isDuplicateUserName = this.appUserReadPlatformService.isUsernameExist(username);
    if (isDuplicateUserName) throw enrollmentConflict("error.msg.user.duplicate.username", "Username already exists", "username");
  }
  ...
  private RuntimeException translateEnrollmentConflict(PlatformDataIntegrityException exception) {
    ...
    if ("error.msg.client.duplicate.mobileNo".equals(code)) return enrollmentConflict(code, exception.getDefaultUserMessage(), "mobileNo");
    if ("error.msg.client.duplicate.email".equals(code)) return enrollmentConflict(code, exception.getDefaultUserMessage(), "email");
    if ("error.msg.user.duplicate.username".equals(code)) return enrollmentConflict(code, exception.getDefaultUserMessage(), "username");
    return exception;
  }
  ...
  private RuntimeException translateEnrollmentFailure(Exception exception) {
    ...
    if (message == null) return ErrorHandler.getMappable(exception, "error.msg.unknown.data.integrity.issue", "Unknown data integrity issue");
    String normalized = message.toLowerCase();
    if (normalized.contains("username") || normalized.contains("username_org")) return enrollmentConflict("error.msg.user.duplicate.username", "Username already exists", "username");
    if (normalized.contains("mobile_no") || normalized.contains("mobileno")) return enrollmentConflict("error.msg.client.duplicate.mobileNo", "Mobile number already exists", "mobileNo");
    if (normalized.contains("email")) return enrollmentConflict("error.msg.client.duplicate.email", "Email already exists", "email");
  ...
  private JsonObject normalizeSelfEnrollmentClientPayload(JsonObject originalJson) {
    ...
    copyFirstPresent(originalJson, sanitizedJson, SelfServiceApiConstants.firstnameParamName, SelfServiceApiConstants.firstnameParamName, SelfServiceApiConstants.firstNameParamName);
    copyFirstPresent(originalJson, sanitizedJson, SelfServiceApiConstants.middlenameParamName, SelfServiceApiConstants.middlenameParamName, SelfServiceApiConstants.middleNameParamName);
    copyFirstPresent(originalJson, sanitizedJson, SelfServiceApiConstants.lastnameParamName, SelfServiceApiConstants.lastnameParamName, SelfServiceApiConstants.lastNameParamName);
    ...
    copyIfPresent(originalJson, sanitizedJson, SelfServiceApiConstants.clientClassificationIdParamName, SelfServiceApiConstants.clientClassificationIdParamName);
    ...
    copyFirstPresent(originalJson, sanitizedJson, SelfServiceApiConstants.externalIdParamName, SelfServiceApiConstants.externalIdParamName, SelfServiceApiConstants.externalIDParamName);
    ...
    int legalFormId = originalJson.has(SelfServiceApiConstants.legalFormIdParamName) ? originalJson.get(SelfServiceApiConstants.legalFormIdParamName).getAsInt() : Integer.parseInt(env.getProperty("fineract.selfservice.enrollment.default-legal-form-id", "1"));
    ...
    if (StringUtils.isBlank(dateFormat)) {
      ...
      else dateFormat = env.getProperty("fineract.selfservice.enrollment.default-date-format", "yyyy-MM-dd");
    }
    sanitizedJson.addProperty(SelfServiceApiConstants.dateFormatParamName, dateFormat);

    String locale = stringValueOrDefault(originalJson, SelfServiceApiConstants.localeParamName, env.getProperty("fineract.selfservice.enrollment.default-locale", "en"));
    ...
    sanitizedJson.addProperty(SelfServiceApiConstants.submittedOnDateParamName, StringUtils.defaultIfBlank(submittedOnDate, today));
  ...
  private SelfServiceRegistration resolveRegistrationRequest(JsonElement element, DataValidatorBuilder baseDataValidator, List<ApiParameterError> dataValidationErrors) {
    String externalToken = this.fromApiJsonHelper.extractStringNamed(SelfServiceApiConstants.externalAuthenticationTokenParamName, element);
    if (externalToken != null) {
      baseDataValidator.reset().parameter(SelfServiceApiConstants.externalAuthenticationTokenParamName).value(externalToken).notBlank().notExceedingLengthOf(100);
      if (!dataValidationErrors.isEmpty()) return null;
      SelfServiceRegistration request = this.selfServiceRegistrationRepository.getRequestByExternalAuthorizationToken(externalToken, SelfServiceRequestType.REGISTRATION);
      validateRequestState(request, externalToken);
      return request;
    }
    Long id = this.fromApiJsonHelper.extractLongNamed(SelfServiceApiConstants.requestIdParamName, element);
    ...
    baseDataValidator.reset().parameter(SelfServiceApiConstants.authenticationTokenParamName).value(authenticationToken).notBlank().notNull().notExceedingLengthOf(100);
    if (!dataValidationErrors.isEmpty()) return null;
    SelfServiceRegistration request = this.selfServiceRegistrationRepository.getRequestByIdAndAuthenticationToken(id, authenticationToken, SelfServiceRequestType.REGISTRATION);
    validateRequestState(request, id, authenticationToken);
    return request;
  }
  ...
  private void validateRequestState(SelfServiceRegistration request, String externalToken) {
    ...
    if (request.isConsumed() || request.isExpired(DateUtils.getLocalDateTimeOfSystem())) throw new PlatformDataIntegrityException("error.msg.self.service.request.token.invalid", "The supplied self-service token is expired or already used.", SelfServiceApiConstants.externalAuthenticationTokenParamName, externalToken);
  }

  private void validateRequestState(SelfServiceRegistration request, Long requestId, String authenticationToken) {
    if (request == null) throw new SelfServiceRegistrationNotFoundException(requestId, authenticationToken);
    if (request.isConsumed() || request.isExpired(DateUtils.getLocalDateTimeOfSystem())) throw new PlatformDataIntegrityException("error.msg.self.service.request.token.invalid", "The supplied self-service token is expired or already used.", SelfServiceApiConstants.authenticationTokenParamName, authenticationToken);
  }
