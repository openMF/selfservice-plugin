...
import org.apache.fineract.selfservice.security.domain.SelfServiceAccessAudit;
import org.apache.fineract.selfservice.security.domain.SelfServiceAccessAuditDto;
import org.apache.fineract.selfservice.security.domain.SelfServiceAccessAuditService;
...
public class SelfServiceOwnershipGuardImpl implements SelfServiceOwnershipGuard {
  ...
  public void validateClientOwnership(final Long clientId) {
    ...
    if (clientId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.CLIENT, null, "null");
    }
  ...
  public void validateSavingsOwnership(final Long savingsAccountId) {
    ...
    if (savingsAccountId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.SAVINGS_ACCOUNT, null, "null");
    }
  ...
  public void validateLoanOwnership(final Long loanId) {
    ...
    if (loanId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.LOAN_ACCOUNT, null, "null");
    }
  ...
  public void validateShareOwnership(final Long shareAccountId) {
    ...
    if (shareAccountId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.SHARE_ACCOUNT, null, "null");
    }
    ...
    if (!isMapped) {
      ...
      throw new ShareAccountNotFoundException(shareAccountId);
    }
  ...
  public void validateTransferSourceOwnership(
    ...
    if (StringUtils.isBlank(accountIdentifier)) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.TRANSFER_SOURCE, null, "blank");
    }
  ...
  public void validateBeneficiaryOwnership(final Long beneficiaryId) {
    ...
    if (beneficiaryId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.BENEFICIARY, null, "null");
    }
    ...
    final Boolean isOwned =
        jdbcTemplate.queryForObject(
            """
                ...
                WHERE id = ? AND app_user_id = ? AND is_active = TRUE
                """,
            Boolean.class,
            beneficiaryId,
            user.getId());
  ...
  public void validatePocketOwnership(final Long mappingId) {
    ...
    if (mappingId == null) {
      throw denyAndThrow(user, SelfServiceAccessAudit.ResourceType.POCKET, null, "null");
    }
  ...
  private Long resolveAccountIdForOwnershipCheck(
    ...
    try {
      ...
      // Try as numeric ID first, but ONLY if it doesn't look like a zero-padded account number
  ...
  private void enforceRateLimit(
    ...
    if (auditService.isRateLimitExceeded(user.getId(), resourceType)
      ...
      log.warn(
          "SECURITY: Rate limit exceeded for userId={} resourceType={}. Blocking.",
          user.getId(),
          resourceType);
  ...
  /** Records a DENIED audit event and returns the unified exception to be thrown. */
  private SelfServiceAccessDeniedException denyAndThrow(
    ...
    return new SelfServiceAccessDeniedException(user.getId(), resourceType.name(), resourceId);
  }
